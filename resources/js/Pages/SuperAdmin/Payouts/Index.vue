...
const handleView = (row) => {
    router.visit(`/super-admin/payouts/${row.id}`);
};

const handleProcess = async (payout) => {
    if (!confirm(`Process payout of $${payout.amount} to ${payout.teacher}?`)) return;
    
    try {
        await api.post(`/payouts/${payout.id}/process`);
        await fetchPayouts();
        await fetchStats();
    } catch (error) {
...
const handleBulkProcess = async () => {
    if (!confirm(`Process ${selectedRows.value.length} payout(s)?`)) return;
    
    try {
        await Promise.all(selectedRows.value.map(payout => api.post(`/payouts/${payout.id}/process`)));
        selectedRows.value = [];
        await fetchPayouts();
        await fetchStats();
    } catch (error) {
    ...
    <AuthenticatedLayout>
        <div class="space-y-6 pb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Payouts Management</h1>
                    <p class="mt-1 text-sm text-gray-500">
                        Manage teacher payouts and salary payments
                    </p>
                </div>
                <Button @click="() => router.visit('/super-admin/payouts/process')" variant="primary">
                ...
                <Card class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-yellow-600">Requires processing</p>
                        </div>
            ...
            >
                ...
                <template #cell-classes="{ row }">
                    <Badge variant="secondary">{{ row.classes || 0 }}</Badge>
                </template>
                ...
                <template #row-actions="{ row }">
                    <div class="flex items-center justify-end gap-2">
                        ...
                        <button
                            ...
                            class="rounded-lg px-3 py-1.5 text-xs font-medium text-white bg-green-600 hover:bg-green-700 transition-colors"
