...
const fetchRecordings = async () => {
    loading.value = true;
    try {
        const { data } = await api.get('/teacher/recordings');
...
const handleUpload = () => {
    router.visit('/teacher/recordings/upload');
};

const handleView = (row) => {
    router.visit(`/teacher/recordings/${row.id}`);
};
    ...
    <AuthenticatedLayout>
        <div class="space-y-6 pb-8">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">My Recordings</h1>
                    <p class="mt-1 text-sm text-gray-500">
                        Upload and manage your class recordings
                    </p>
                </div>
