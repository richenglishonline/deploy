...
const fetchStudents = async () => {
    ...
    try {
        ...
        students.value = Array.isArray(data.students || data) ? (data.students || data).map(student => ({
            ...student,
...
const handleView = (row) => {
    router.visit(`/admin/students/${row.id}`);
};

const handleEdit = (row) => {
    router.visit(`/admin/students/${row.id}/edit`);
};
...
const handleBulkDelete = async () => {
    if (!confirm(`Are you sure you want to delete ${selectedRows.value.length} student(s)?`)) return;
    
    try {
        await Promise.all(selectedRows.value.map(student => api.delete(`/admin/students/${student.id}`)));
        selectedRows.value = [];
        await fetchStudents();
        await fetchStats();
    } catch (error) {
